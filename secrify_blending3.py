...
def get_count_aa(seq,aa_to_count):
    ...
    for aa in aa_to_count:
        total += seq.count(aa)/float(len(seq))
    return(total)
...
def get_set_all_aa(infile_name,ret_duplicated_pos=False,ignore_idents=set()):
    infile = open(infile_name)
    ...
    for line in infile:
        ...
        if splitline[0] in ignore_idents: continue
...
def get_feats_simple(infile_name,libs_prop,assign_class=1,nmer_feature="nmer_features_new.txt"):
    infile = open(infile_name)
    nmers = [nmf.rstrip() for nmf in open(nmer_feature).readlines()]
...
def train_xgb_cv(X,y,mod_number=1,cv=None,outfile="mod.pickle"):
    ...
    xgb_handle = xgb.XGBClassifier(nthread=-1)
    ...
    param_dist = {  
        "n_estimators": [1],#st.randint(5,25), #st.randint(5, 25) #50
        "max_depth": [20], #st.randint(2, 4), #st.randint(3, 8) #6
        "tree_method" : ["hist"]
    ...
    #param_dist = {  
    #    "n_estimators": st.randint(5, 75),#st.randint(5,25), #st.randint(5, 75) #50
    ...
    while len(all_feats) > 2:
        ...
        for f_name,scores in score_dict.items():
            avg_perf = sum(score_dict[f_name])/len(score_dict[f_name])
            print("%s,%s,%s" % (f_name,avg_perf,best_perf))
            if avg_perf > best_perf:
                best_perf = avg_perf
                rem_feat = f_name
        all_feats.remove(rem_feat)
    ...
    for ind,test_set in enumerate(alt_cv):
        ...
        random_search = RandomizedSearchCV(xgb_handle, param_distributions=param_dist,
                                           n_iter=n_iter_search,verbose=0,scoring="roc_auc", #["accuracy",]
                                           n_jobs=24,refit=True,cv=cv_temp) # StratifiedKFold(n_splits=5, shuffle=True,random_state=42))
...
def train_xgb(X,y,mod_number=1,cv=None,outfile="ensemble.pickle"):
    ...
    for train_indexes,test_indexes in cv:
        ...
        all_te_indices.extend(test_indexes)
    ...
    xgb_model = random_search.best_estimator_
    pickle.dump(random_search,open(outfile,"wb"))
    return(zip(all_te_indices,y_vals,preds))
...
def get_feats(main_path="",infile_name="data/train.csv",assign_class=0):    
    ...
    for seq in all_seqs:
        ...
        seq_ten_splits_s = list(split_seq(seq,6))
        #seq_ten_splits.extend([seq[0:5],seq[-5:]])
        seq_ten_splits = [seq_ten_splits_s[0],seq_ten_splits_s[1],seq_ten_splits_s[2],seq_ten_splits_s[3],seq_ten_splits_s[4],seq_ten_splits_s[5],seq[0],seq[1],seq[2],seq[3],seq[4],seq[5],seq[-1],seq[-2],seq[-3],seq[-4],seq[-5],seq[-6]]
        instance_dict = {}
    ...
    return(pd.DataFrame(data_df,index=rownames_depleted),feats)
...
def get_preds_ensemble(main_path="C:/Users/asus/Dropbox/secretome/kaggle/"):
    ...
    test_df[feats].to_csv("first_layer_submission.csv")
    ...
    test_df["mean_prediction"] = preds_df.mean(axis=1)
    #preds_df["mod0"]
    test_df[["mean_prediction","med_prediction","prediction"]].to_csv("submission.csv")
    plt.hist(preds_df.std(axis=1),bins=500)
    plt.show()
    #print(preds_df.std(axis=1))
    plt.hist(preds_df.median(axis=1),bins=500)
...
def plot_feats(X,y):
    for f1 in X.columns:
        for f2 in X.columns:
            if f1 == f2: continue
            #sns.kdeplot(X[f1][y==0],X[f2][y==0],shade=True,n_levels=100,cmap="Blues")
            #sns.kdeplot(X[f1][y==1],X[f2][y==1],shade=True,n_levels=100,cmap="Reds")
            #plt.show()
            #plt.close()

def train_all_instances(main_path="",input_depleted="data/train_depleted_all_50.csv",input_enriched="data/train_enriched_all_50.csv",random_seed=42):
    #identifiers_depleted = [l.split(",")[0] for l in open(input_depleted).readlines() if not l.startswith("identifier")]
    #identifiers_enriched = [l.split(",")[0] for l in open(input_enriched).readlines() if not l.startswith("identifier")]
    #identifiers_enriched.extend(identifiers_depleted)
    #print(identifiers_depleted)
    #input()
    
    outfile_log = open("outlog.txt","a")
    ...
    #data_df.extend(data_df_enriched)
    #data_df.extend(data_df_depleted)
    pd_df = pd.concat([data_df_enriched,data_df_depleted])
    index_to_ident = dict(zip(list(range(len(pd_df.index))),list(pd_df.index)))

    """
    pd_df_rewrite = pd.read_csv("sc_50/output_ensemble_preds.csv",index_col=0)

    print(pd_df_rewrite)

    pd_df_rewrite.index = [index_to_ident[ind] for ind in pd_df_rewrite["ind"]]

    pd_df_rewrite.to_csv("sc_50/output_ensemble_preds_correct_index.csv")
    print(pd_df_rewrite)
    ...
    #pd_df.index = identifiers_enriched
    #print(len(pd_df))
    #to_remove = []
    #for i,v in enumerate(data_df):
    #    if v["seq_length"] < 0:
    #        to_remove.append(i)
    #for i in to_remove[::-1]:
    #    del data_df[i]
        
    #pd_df = pd_df.loc[pd_df["seq_length"] > 0,:]
    #input()
    pd_df.index = list(range(len(pd_df.index)))
    #pd_df = pd_df.sample(10000)

    #feats_sven = ["Ensembl_geneID","class"]
    #feats_sven.extend(open("selected_features_sven.csv").readlines())
    #feats_sven = [f.rstrip() for f in feats_sven]
    
    #print(list(pd_df.columns))
    #input("selection2")
    #pd_df[feats_sven].to_csv("matrix_feats_filtered.csv")
    #pd_df.to_csv("matrix_feats.csv")
    #input("selection")
    ...
    folds = div_into_folds(gene_idents_sorted,n_folds=3) #5
    ...
    """
    cv = list(set(list(itertools.chain(*[list(cv1) for cv1,cv2 in cv_new]))))
    random.shuffle(cv)
    
    dists = int(len(cv)/5.0)
    folds = [cv[i:i + dists] for i in range(0, len(cv), dists)]
    if len(folds) > 5:
        folds[4].extend(folds[5])
    folds = folds[0:5]
    cv = []
    ...
    #input()
    #cv = make_CV(cv,ident_to_indices)
    """
    ...
    for index,f_group in enumerate(feat_groups):
        ...
        """
        selected_feats_old = [f for f in feats if f.startswith(f_group)]
        #selectie = ["|0","|6","|7","|8","|9","|10","|12","|13","|14","|15","|16"]
        selectie = ["|6","|12"]
        selected_feats = []
        for sf in selected_feats_old:
            if "|" not in sf: 
                selected_feats.append(sf)
                continue
            if "|"+sf.split("|")[1] in selectie:
                selected_feats.append(sf)
        """
        selected_feats.append("seq_length")
        #selected_feats.append("organism")
        #
        #selected_feats.extend(["L","R","K","T","E","F","S","C","Y","D"]) #feat_aa
        #selected_feats.extend(["L","R","K"])
    ...
    #feats_ensemble = ['L','R','K','seq_length','mod_xgb_0', 'mod_xgb_1', 'mod_xgb_2', 'mod_xgb_3', 'mod_xgb_4', 'mod_xgb_5', 'mod_xgb_6', 'mod_xgb_7', 'mod_xgb_8', 'mod_xgb_9']
    X = pd_df[feats_ensemble]
    y = pd_df["class"]

    

    pd_pred = train_xgb(X,y,mod_number=1,cv=cv,outfile="ensemble.pickle")
    pd_pred = pd.DataFrame(list(pd_pred),columns=["ind","y","pred"])
    pd_pred.index = [index_to_ident[ind] for ind in pd_pred["ind"]]
    pd_pred.to_csv("output_ensemble_preds.csv") #,index=False)

    pd_df.index = [index_to_ident[ind] for ind in pd_df.index]
    
    pd_df[['class','mod_xgb_0', 'mod_xgb_1', 'mod_xgb_2', 'mod_xgb_3', 'mod_xgb_4', 'mod_xgb_5', 'mod_xgb_6', 'mod_xgb_7', 'mod_xgb_8', 'mod_xgb_9']].to_csv("layer_one_predictions.csv")
